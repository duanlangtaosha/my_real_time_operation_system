...
//		ls_flag_group_get_flag(&flag_group, LS_FLAG_SET_ALL, 0x01, &result_inof);
		...
		ls_task_get_info(current_task, &task1_info);
	}
}
...
//		ls_flag_group_delete(&flag_group);
		flag2 = 0;
		ls_delayms(2);
	ls_task_get_info(current_task, &task2_info);
...
void task3_func()
{
	for (; ;) {
		...
		flag3 = 0;
		ls_delayms(2);
		ls_task_get_info(current_task, &task3_info);
	}
}
...
void task4_func()
{
	for (; ;) {
		...
		flag4 = 0;
		ls_delayms(2);
		ls_task_get_info(current_task, &task4_info);
	}
}
